...
def quantize_tensor(tensor, bitwidth, channel_wise=False, rescale_back=False):
    if channel_wise:  # 逐通道量化
        _max = tensor.abs().view(tensor.size(0), -1).max(1)[0]  # 获取每一行的最大绝对值
    else:  # 逐层量化
        _max = tensor.abs().max()  # 获取最大绝对值
    ...
    new_tensor = torch.round(tensor / alpha)  # 四舍五入
    if rescale_back:
...
def quantize_layers(rescale_back=True):
    for layer, quantization_var in layer_quantization_vars.items():
        with torch.no_grad():
            ...
            #layer.weight.data为模拟量化（量化+逆量化）后的，layer.weight.weight_float为量化前的浮点数
